...
- **Import Concurrency Control** - **Business Value**: Prevents multiple simultaneous imports that could corrupt data or create race conditions. **Current Status**: Backend handles atomic transactions correctly, but UI/API should enforce single-import-at-a-time policy. **Implementation**: Add import lock at API level - return "Import already in progress" error if one is running. Show status indicator in UI when import is active. Prevents user from accidentally starting second import while first is processing. **Technical Effort**: 2-3 hours (API middleware, UI state management). **Priority**: Medium - important for production but single-user MVP scenario has low risk of concurrent imports.
