...
Our search system uses 5 optimization techniques to deliver fast results without requiring Redis, Elasticsearch, or other complex infrastructure. This guide explains each technique, shows the actual implementation, and demonstrates how to measure performance.

**Prerequisites**: Read [Overview](index) to understand the 6-stage cascade architecture.

### The Big Picture

Our search system handles three types of operations, each with different performance characteristics:

**1. SKU search** - Find parts by part number (ACR or competitor SKU). This is the most common operation, representing approximately 80% of all searches. Parts counter staff scan barcodes or copy-paste part numbers from customer orders, so these searches need to be instant. Any delay impacts the sale since customers are waiting at the counter.

**2. Vehicle search** - Find parts that fit a specific vehicle (make, model, year). These searches are less frequent but return larger result sets (typically 20-50 matching parts). Users browse through paginated results to find the right part for their vehicle.

**3. Image enrichment** - Attach product photos to search results. Every search result needs at least one image (the primary product photo). Without optimization, fetching images for 15 search results would require 16 separate database queries (1 for parts + 15 for images), creating significant latency.

### Core Optimization Philosophy

We optimize for real-world usage patterns observed in production. The majority of searches (~80%) are exact ACR SKU lookups where users scan a barcode or copy-paste a part number. These need to be instant - parts counter staff are helping customers in real-time and any delay impacts the customer experience and sale completion.

The remaining searches fall into two categories: competitor SKU lookups (~15%) where staff receive a part from another manufacturer and need to find the ACR equivalent, and typo-tolerant searches (~5%) where someone manually types a part number and makes a small mistake like "ACR230004" instead of "ACR2303004".

This usage pattern drives our architectural decisions. We optimize the common case (exact matches) to be lightning-fast using indexed lookups that complete in single-digit milliseconds. Less common cases (competitor SKUs) use efficient joins and are still fast. Edge cases (typos) use fuzzy matching which is slower but acceptable since they're rare. The 6-stage cascade pattern implements this through early termination - if Stage 1 finds an exact match, Stages 2-6 never execute.

**Key principle**: Make the common case fast, keep the less common cases acceptable, and ensure edge cases still work.

### How We Achieve Fast Searches

We use five complementary techniques that work together:

**1. Database indexes** - Pre-built data structures that let Postgres jump directly to matching rows instead of scanning entire tables. We maintain 12 indexes across 3 tables (parts, cross_references, vehicle_applications) covering both exact matches (B-tree indexes) and fuzzy matching (GIN trigram indexes).

**2. Early termination (cascade pattern)** - The search function checks 6 different strategies in order of likelihood, but stops immediately when it finds a match. For the common case (exact ACR SKU), only Stage 1 executes and Stages 2-6 never run. This means 80% of searches take the fastest possible path.
...
**Result**: Sub-100ms searches for the common case, using only Postgres and smart query patterns. No Redis, no Elasticsearch, no additional infrastructure to maintain.

---

## Optimization Techniques

**All techniques below are implemented in production code** with accurate file references.
...
```sql
-- File: supabase/migrations/20250907000000_initial_schema.sql (lines 84-95)
...
```sql
-- File: supabase/migrations/20251109000000_add_sku_normalization.sql (lines 178-190)
...
```sql
-- File: supabase/migrations/20251109000000_add_sku_normalization.sql (lines 209-303)
...
// File: src/app/api/public/parts/route.ts (lines 10-78)
...
| Approach | Queries | Approximate Time |
|----------|---------|------------------|
| N+1 (bad) | 1 + 15 = **16 queries** | ~800ms (50ms × 16) |
| Batch (good) | 1 + 1 = **2 queries** | ~100ms (50ms × 2) |
| **Improvement** | **8x fewer queries** | **8x faster** |
...
**Why it matters**: Parts counter staff often search the same SKUs repeatedly throughout the day. Caching makes repeat searches instant (0ms, no network request).
...
```typescript
// File: src/hooks/api/public/parts.ts (lines 52-98)
...
|-------------|--------------|----------------------------|
| ACR SKU | ~100ms (server query) | **0ms** (cached) |
| Vehicle | ~150ms (server query) | **0ms** (cached) |
| Different SKU | ~100ms (new query) | ~100ms (different cache key) |
...
**Real-world example**:
1. User searches "ACR2303004" → 100ms (server)
2. User clicks part → views details → clicks back
3. User searches "ACR2303004" again → **0ms (cached)** ✅
4. User searches "TM515072" → 100ms (different cache key, new query)
...
```typescript
// File: src/app/api/public/parts/route.ts (lines 198-220)
...
```typescript
// File: src/app/api/public/parts/route.ts (lines 170-194)
