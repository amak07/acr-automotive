...
How the dual-mode search system (vehicle + SKU) works with normalization, fuzzy matching, and sub-300ms performance.

**When to use this guide**:

- Building similar search features with multi-stage fallback algorithms
- Understanding the architecture before making changes
- Learning RPC functions vs client-side queries
...
1. **Fast**: Sub-second response time with indexed queries
...
<Mermaid chart={`sequenceDiagram
    ...
    DB-->>API: Return parts (50-180ms)
    API->>Storage: Batch fetch images (.in(partIds))
    Storage-->>API: Return image URLs (30ms)
...
Location: [supabase/migrations/20251109000000_add_sku_normalization.sql](../../../supabase/migrations/20251109000000_add_sku_normalization.sql#L209-L304)
...
Location: [src/app/api/public/parts/route.ts](../../../src/app/api/public/parts/route.ts#L198-L221)
...
- `staleTime` = 5 min means repeat searches hit cache (0ms)
- `gcTime` = 10 min means cached data survives navigation

#### Component Four - Validation Schema `publicSearchSchema`

Location: [src/lib/schemas/public.ts](../../../src/lib/schemas/public.ts#L1-L12)
