...
type VehicleAppRow = Database['public']['Tables']['vehicle_applications']['Row'];
type CrossRefRow = Database['public']['Tables']['cross_references']['Row'];

// Configuration
const UNIT_OUTPUT_DIR = path.join(process.cwd(), 'fixtures', 'excel', 'unit');
const SCENARIOS_OUTPUT_DIR = path.join(process.cwd(), 'fixtures', 'excel', 'scenarios');
 ...
 */
async function saveWorkbook(workbook: ExcelJS.Workbook, directory: string, filename: string) {
...
async function generateFixture1_ValidAddNewParts() {
  ...
  const newParts = [
    ['', 'ACR-TEST-001', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Test part 1'],
    ['', 'ACR-TEST-002', 'MAZA', 'TRASERA', 'P/ABS', '5 BIRLOS', '5x10', 'Test part 2'],
    ['', 'ACR-TEST-003', 'ROTOR', 'DELANTERA', 'C/ABS', '4 BIRLOS', '', 'Test part 3'],
...
async function generateFixture2_ValidUpdateExisting() {
  ...
  const existingParts = [
    ['existing-id-1', 'ACR123456', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Updated specifications'],
    ['existing-id-2', 'ACR234567', 'ROTOR', 'TRASERA', 'P/ABS', '5 BIRLOS', '5x10', 'Updated specifications'],
    ['existing-id-3', 'ACR345678', 'BALATA', '', '', '', '', 'Updated specifications'],
  ];
...
async function generateFixture3_ErrorDuplicateSkus() {
  ...
  const duplicateParts = [
    ['', 'ACR-DUPLICATE', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'First part'],
    ['', 'ACR-DUPLICATE', 'ROTOR', 'TRASERA', 'P/ABS', '5 BIRLOS', '5x10', 'Duplicate SKU!'], // E2 error
...
async function generateFixture4_ErrorMissingRequiredFields() {
  ...
  const invalidParts = [
    ['', '', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Missing ACR_SKU'], // E3: Missing ACR_SKU
  ...
  await saveWorkbook(workbook, UNIT_OUTPUT_DIR, 'error-missing-required-fields.xlsx');
}
...
async function generateFixture5_ErrorOrphanedReferences() {
  ...
  partsSheet.addRow(['real-part-id', 'ACR-VALID-001', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Valid part']);

  // Vehicle Applications with orphaned part_id
  const vehiclesSheet = addWorksheet(workbook, SHEET_NAMES.VEHICLE_APPLICATIONS, VEHICLE_APPLICATIONS_COLUMNS);
  vehiclesSheet.addRow(['va-id-1', 'nonexistent-part-id', 'ACR-INVALID', 'FORD', 'F-150', 2020, 2023]); // E5: Orphaned FK

  addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, CROSS_REFERENCES_COLUMNS);

  await saveWorkbook(workbook, UNIT_OUTPUT_DIR, 'error-orphaned-references.xlsx');
}
...
async function generateFixture6_ErrorInvalidFormats() {
  ...
  partsSheet.addRow(['invalid-uuid-format', 'ACR-TEST-001', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Invalid ID format']); // E4

  const vehiclesSheet = addWorksheet(workbook, SHEET_NAMES.VEHICLE_APPLICATIONS, VEHICLE_APPLICATIONS_COLUMNS);
  vehiclesSheet.addRow(['va-id-1', 'part-id-1', 'ACR-TEST-001', 'FORD', 'F-150', 2025, 2020]); // E6: start_year > end_year
  vehiclesSheet.addRow(['va-id-2', 'part-id-1', 'ACR-TEST-001', 'FORD', 'F-150', 1899, 1900]); // E8: year out of range
...
async function generateFixture7_ErrorMaxLengthExceeded() {
  ...
  partsSheet.addRow(['', 'ACR-TEST-001', longString, 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Part type too long']); // E7

  addWorksheet(workbook, SHEET_NAMES.VEHICLE_APPLICATIONS, VEHICLE_APPLICATIONS_COLUMNS);
  addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, CROSS_REFERENCES_COLUMNS);

  await saveWorkbook(workbook, UNIT_OUTPUT_DIR, 'error-max-length-exceeded.xlsx');
}
...
async function generateFixture8_WarningDataChanges() {
  ...
  const changedParts = [
    ['existing-id-1', 'ACR-CHANGED-SKU', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Changed SKU'], // W1: ACR_SKU changed
    ['existing-id-2', 'ACR234567', 'ROTOR-CHANGED', 'TRASERA', 'P/ABS', '5 BIRLOS', '5x10', 'Changed type'], // W3: Part_Type changed
    ['existing-id-3', 'ACR345678', 'BALATA', 'DELANTERA-CHANGED', '', '', '', 'Changed position'], // W4: Position_Type changed
  ];
...
async function generateIntegrationScenarios() {
  ...
  try {
    ...
    console.error('Run "npm run test:export-baseline" first to create baseline');
    return;
  }
...
async function loadBaseline(): Promise<BaselineData> {
  ...
  console.log(`  Loaded ${parts.length} parts, ${vehicles.length} vehicles, ${crossRefs.length} cross-refs\n`);
...
async function generateScenario1_QuarterlyUpdate(baseline: BaselineData) {
  ...
  firstRowValues[firstRowValues.length - 1] = 'UPDATED SPECIFICATIONS - Q1 2025';
  ...
  const crossRefsSheet = addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, CROSS_REFERENCES_COLUMNS);
  baseline.crossRefs.forEach(row => crossRefsSheet.addRow(row));

  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, '01-quarterly-update.xlsx');
}
...
async function generateScenario2_SeasonalRefresh(baseline: BaselineData) {
  ...
  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, '02-seasonal-refresh.xlsx');
}
...
async function generateScenario3_MinorCorrections(baseline: BaselineData) {
  ...
  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, '03-minor-corrections.xlsx');
}
...
async function generateScenarioError_E1_MissingIds(baseline: BaselineData) {
  ...
  const vehicleColumnsNoId = VEHICLE_APPLICATIONS_COLUMNS.filter(col => !col.hidden);
  const crossRefColumnsNoId = CROSS_REFERENCES_COLUMNS.filter(col => !col.hidden);

  const partsSheet = addWorksheet(workbook, SHEET_NAMES.PARTS, partsColumnsNoId as any);
  baseline.parts.slice(0, 10).forEach(row => {
    const rowWithoutId = row.slice(1); // Remove _id
    partsSheet.addRow(rowWithoutId);
  });

  const vehiclesSheet = addWorksheet(workbook, SHEET_NAMES.VEHICLE_APPLICATIONS, vehicleColumnsNoId as any);
  const crossRefsSheet = addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, crossRefColumnsNoId as any);

  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, 'error-e1-missing-ids.xlsx');
}
...
async function generateScenarioError_E2_DuplicateSku(baseline: BaselineData) {
  ...
  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, 'error-e2-duplicate-sku.xlsx');
}
...
async function generateScenarioError_E3_EmptyFields(baseline: BaselineData) {
  ...
  partsSheet.addRow(['', '', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Missing SKU']);
  ...
  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, 'error-e3-empty-fields.xlsx');
}
...
async function generateScenarioError_E5_OrphanedFk(baseline: BaselineData) {
  ...
  vehiclesSheet.addRow(['va-id-1', 'nonexistent-part-id', 'ACR-INVALID', 'FORD', 'F-150', 2020, 2023]);

  const crossRefsSheet = addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, CROSS_REFERENCES_COLUMNS);

  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, 'error-e5-orphaned-fk.xlsx');
}

// Error Scenario: E6 - Invalid Year Range
async function generateScenarioError_E6_InvalidYearRange(baseline: BaselineData) {
  const workbook = createWorkbook();

  const partsSheet = addWorksheet(workbook, SHEET_NAMES.PARTS, PARTS_COLUMNS);
  partsSheet.addRow(['part-id-1', 'ACR-VALID-001', 'MAZA', 'DELANTERA', 'C/ABS', '4 BIRLOS', '4x8', 'Valid part']);

  const vehiclesSheet = addWorksheet(workbook, SHEET_NAMES.VEHICLE_APPLICATIONS, VEHICLE_APPLICATIONS_COLUMNS);
  // start_year > end_year
  vehiclesSheet.addRow(['va-id-1', 'part-id-1', 'ACR-VALID-001', 'FORD', 'F-150', 2025, 2020]);

  const crossRefsSheet = addWorksheet(workbook, SHEET_NAMES.CROSS_REFERENCES, CROSS_REFERENCES_COLUMNS);

  await saveWorkbook(workbook, SCENARIOS_OUTPUT_DIR, 'error-e6-invalid-year-range.xlsx');
}
...
async function main() {
  ...
  console.log('📊 Schema-aware: Using shared constants from src/services/excel/shared/');
  console.log('');
