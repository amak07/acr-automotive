...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.error('   Make sure .env.test file exists with:');
...
async function generateAddFixture() {
  ...
  const outputPath = path.join(process.cwd(), 'fixtures', 'excel', 'unit', 'valid-add-new-parts.xlsx');
...
async function generateUpdateFixture() {
  ...
  const { data: parts, error } = await supabase
    .from('parts')
    .select('*')
    .in('acr_sku', ['ACR-TEST-001', 'ACR-TEST-002', 'ACR-TEST-003', 'ACR-TEST-004', 'ACR-TEST-005'])
  ...
  const outputPath = path.join(process.cwd(), 'fixtures', 'excel', 'unit', 'valid-update-existing.xlsx');
  await workbook.xlsx.writeFile(outputPath);

  console.log('✅ Created: valid-update-existing.xlsx');
  console.log('   Expected: 0 adds, ~3 updates, 0 deletes');
  console.log(`   Parts to be updated: ${parts.slice(0, 3).map(p => p.acr_sku).join(', ')}`);
  console.log(`   Parts unchanged: ${parts.slice(3).map(p => p.acr_sku).join(', ')}\n`);
}
...
async function main() {
  ...
  console.log('   2. Run: npx tsx scripts/test/generate-test-parts-with-uuids.ts update');
