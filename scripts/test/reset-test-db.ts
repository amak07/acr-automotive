...
async function resetTestDatabase() {
  ...
  try {
    ...
    if (!goldenSnapshot || findError) {
      ...
      const { setupBaselineSnapshot } = await import(
    ...
    console.log(
      `   Created: ${new Date(goldenSnapshot.created_at).toLocaleString()}\n`
    );
    ...
    await supabase.from("import_history").delete().neq("id", goldenSnapshot.id);
    ...
    if (
      snapshotData.vehicle_applications &&
      snapshotData.vehicle_applications.length > 0
    ) {
      ...
      console.log(
        `   ✅ Restored ${snapshotData.vehicle_applications.length} vehicle applications`
      );
    }

    // Restore cross references
    if (
      snapshotData.cross_references &&
      snapshotData.cross_references.length > 0
    ) {
      ...
      console.log(
        `   ✅ Restored ${snapshotData.cross_references.length} cross references`
      );
    }
    ...
    console.log("═".repeat(80));
    console.log(
      "Your test database has been restored to the baseline state (865 parts)."
    );
    console.log("");
  } catch (error: any) {
