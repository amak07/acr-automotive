---
title: "Phase 3: Technical Foundation (Backend)"
---
...
async function generateEmbeddings() {
  ...
  const { data: parts, error } = await supabase
    .from("parts")
    .select(
      "id, acr_sku, part_type, position_type, abs_type, bolt_pattern, drive_type, specifications"
    )
  ...
  for (let i = 0; i < parts.length; i += batchSize) {
    ...
    try {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        if (updateError) {
          console.error(
            `❌ Error updating part ${batch[j].acr_sku}:`,
            updateError
          );
        }
      }
...
const AISearchRequestSchema = z.object({
  ...
  conversationHistory: z
    ...
    .max(10)
    .optional(), // Limit history to prevent large payloads
});
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          aiMessage:
            "Has alcanzado el límite de búsquedas. Intenta en 10 minutos o usa búsqueda tradicional.",
      ...
      return NextResponse.json(
        {
          ...
          aiMessage:
            "He alcanzado el límite de refinamientos. Intenta búsqueda por filtros.",
    ...
    if (intentResult.intent === "off_topic") {
      return NextResponse.json({
        ...
        aiMessage:
          "Soy el asistente de rodamientos MAZA de ACR. ¿Necesitas ayuda para encontrar una parte?",
        conversationComplete: true,
        processingTime: Date.now() - startTime,
      });
    }
    ...
    if (intentResult.intent === "non_maza_parts") {
      return NextResponse.json({
        ...
        aiMessage:
          "Somos especialistas en rodamientos MAZA. ¿Necesitas rodamientos de rueda para tu vehículo?",
        conversationComplete: true,
        processingTime: Date.now() - startTime,
      });
    }
    ...
    return NextResponse.json(
      {
        messageType: "error",
        aiMessage:
          "La búsqueda inteligente no está disponible. Mostrando resultados tradicionales.",
        fallbackUsed: true,
        showTraditionalSearch: true,
        processingTime: Date.now() - startTime,
      },
...
export async function searchHybrid(
  ...
  if (
    intentResult.intent === "vehicle_search" &&
...
async function searchByVehicle(make: string, model: string, year: number) {
  const { data, error, count } = await supabase.rpc("search_by_vehicle", {
    make,
    model,
    target_year: year,
  });
...
async function searchBySKU(sku: string) {
  const { data, error, count } = await supabase.rpc("search_by_sku", {
    search_sku: sku,
  });
...
export async function getDailyCostSummary(date: Date) {
  ...
  const totalCost =
    data?.reduce((sum, log) => sum + log.estimated_cost, 0) || 0;
...
const COST_THRESHOLDS = {
  daily: 1.0, // Alert if >$1/day
  weekly: 5.0, // Alert if >$5/week
  monthly: 15.0, // Alert if >$15/month
};
