...
export async function GET() {
  try {
    // Get all vehicle combinations in one query
    ...
    const modelsGrouped = vehicles.reduce((acc, v) => {
      if (!acc[v.make]) {
        acc[v.make] = new Set();
      }
    ...
    const models = Object.keys(modelsGrouped).reduce((acc, make) => {
      acc[make] = [...modelsGrouped[make]].sort();
      return acc;
    }, {} as { [make: string]: string[] });

    // Group years by make-model combination
    const yearsGrouped = vehicles.reduce((acc, v) => {
    ...
    // Convert Sets to sorted arrays
    const years = Object.keys(yearsGrouped).reduce((acc, key) => {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch vehicle options',
        timestamp: new Date().toISOString()
      },
      { status: 500 }
    );
