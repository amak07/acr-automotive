...
import { DatabasePartRow, DatabaseCrossRef } from "./utils";
...
export async function testImportAcrSkusAsParts(
  ...
  console.log(`✅ [TEST] Inserted ${importedParts.data.length} parts to test schema`);
...
export async function testImportCrossReferences(
  ...
  console.log("🧪 [TEST] Importing cross-references to test.cross_references table...");
  ...
  crossReferences.forEach((item) => {
    ...
    if (item.competitorSku.length > 50) {
      skippedLongSkus++;
      console.warn(`⚠️  [TEST] Skipping long competitor SKU (${item.competitorSku.length} chars): "${item.competitorSku.substring(0, 50)}..."`);
      return;
    }
  ...
  if (skippedLongSkus > 0 || skippedDuplicates > 0) {
    console.log(`📊 [TEST] Skipped ${skippedLongSkus} cross-references with long SKUs (>50 chars)`);
    if (skippedDuplicates > 0) {
      console.log(`📊 [TEST] Skipped ${skippedDuplicates} duplicate cross-references (data entry cleanup)`);
    }
    console.log(`📊 [TEST] Importing ${databaseCrossRefs.length} unique cross-references to test schema`);
  }
  ...
  console.log(`✅ [TEST] Inserted ${importedCrossRefs.data.length} cross-references to test schema`);
...
export async function testImportPreciosData(preciosResult: PreciosResult) {
  ...
  const insertedCrossRefs = await testImportCrossReferences(crossRefs, insertedParts);
